...
export function ChatInput({
  ...
  return (
    <form 
      ...
      className="relative px-4 py-4 space-y-4 bg-transparent"
    >
      {attachedFiles.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-2">
          {attachedFiles.map((file, index) => (
            <div 
              key={index} 
              className="bg-primary/10 text-sm rounded-md px-2 py-1 flex items-center gap-1"
            >
      ...
      <div className="chat-input flex items-center p-2">
        ...
        <Button
          ...
          className={cn(
            ...
            isGenerating ? "bg-destructive hover:bg-destructive/90" : 
              (input.trim() || attachedFiles.length > 0) ? "bg-primary hover:bg-primary/90" : "text-muted-foreground bg-muted hover:bg-muted/90"
          )}
