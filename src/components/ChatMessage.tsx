...
export function ChatMessage({ message }: ChatMessageProps) {
  ...
  const hasThinking = !isUser && !isSystem && message.content.includes('<think>') && message.content.includes('</think>');
  let thinkingContent = '';
  ...
  if (hasThinking) {
    ...
    if (thinkStart !== -1 && thinkEnd !== -1 && thinkEnd > thinkStart) {
      ...
      mainContent = message.content.substring(0, thinkStart) + 
                   message.content.substring(thinkEnd + 8).trim();
    }
  ...
  useEffect(() => {
    ...
    codeBlocks.forEach((codeBlock, index) => {
      ...
      copyButton.className = 'copy-button absolute top-2 right-2 p-1 rounded-md bg-gray-800 hover:bg-gray-700 transition-colors';
      copyButton.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-gray-400"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg>';
  ...
  return (
    <div className={cn(
      "py-6 px-4 flex items-start gap-4",
