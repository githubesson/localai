...
export function ModelSelector({
    ...
    const selectedModelInfo =
        models && Array.isArray(models) ? models.find((model) => model.id === selectedModel) : undefined;
    ...
    return (
        <DropdownMenu>
            <DropdownMenuTrigger asChild disabled={disabled || isLoading}>
                <Button variant="outline" className="bg-secondary/20 text-sm border-secondary/30 hover:bg-secondary/30">
                    {isLoading ? 'Loading models...' : displayName}
                    <ChevronDown className="ml-2 h-4 w-4" />
                </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent
                align="end"
                className="w-[250px] bg-grok-dark border-secondary/30 max-h-[400px] overflow-y-auto"
            >
                    ...
                    models.map((model) => (
                        ...
                        >
                            <div className="flex items-center w-full">
                                ...
                                {selectedModel === model.id && <Check className="h-4 w-4 text-primary ml-2" />}
                            </div>
                        </DropdownMenuItem>
                    ))}
            </DropdownMenuContent>
