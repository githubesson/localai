...
export default function Chat() {
  ...
  useEffect(() => {
    ...
    const cleanupStorage = createLocalStorageListener('sidebar-collapsed', 
      (newValue) => setSidebarCollapsed(newValue === 'true'));
  ...
  useEffect(() => {
    const updateEmptySessionPrompt = (newPrompt: string) => {
      if (newPrompt !== lastDefaultSystemPrompt && 
    ...
    const cleanupStorage = createLocalStorageListener('localai-system-prompt', 
      (newValue) => updateEmptySessionPrompt(newValue || ''));
  ...
  useEffect(() => {
    ...
    if (sessions.length > 0 && !currentSessionId) {
      ...
      const modelToUse = lastSelectedModel && availableModels.some(model => model.id === lastSelectedModel)
        ? lastSelectedModel
  ...
  }, [sessionsLoaded, modelsLoaded, sessions, currentSessionId, models, setCurrentSessionId, getLastSelectedModel, createNewSession]);
  ...
  const handleCreateNewSession = () => {
    if (Array.isArray(models) && models.length > 0) {
      ...
      const modelToUse = lastSelectedModel && models.some(model => model.id === lastSelectedModel)
        ? lastSelectedModel
        : models[0].id;
  ...
  const handleSendMessage = (content: string, attachedFiles?: File[]) => {
    ...
    if (attachedFiles && attachedFiles.length > 0) {
      const fileInfos = attachedFiles.map(file => 
        `📎 ${file.name} (${(file.size / 1024).toFixed(2)} KB, type: ${file.type})`
      );
      ...
      <div className="flex flex-col flex-1 h-full overflow-hidden">
        ...
        {currentSession ? (
          <>
            ...
            <div className={`absolute bottom-0 right-0 transition-all duration-300 ${sidebarCollapsed ? 'left-16' : 'left-64'}`}>
          ...
          <div className="flex flex-col items-center justify-center h-full">
            ...
            <Button
              className="mt-4 bg-grok-input hover:bg-grok-input/90"
